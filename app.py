...
def on_image_upload(image, backend_type):
    if backend_type == "CLIP":
        with torch.no_grad():
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        backend_type = gr.Dropdown(["CLIP", "AlexNet", "ResNet", "ViT", "DeiT"], label="Model")
        nprobe_slider = gr.Slider(minimum=1, maximum=20, value=1, step=1, label="Probes")
        display = gr.Text(label="Info")

    image_input.upload(on_image_upload, inputs=[image_input, backend_type], outputs=image_gallery)
